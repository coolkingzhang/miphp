...
class mysqliAdapter
{
    ...
    function connect($link_type = 'read')
    {
        ...
        $this->conn = new mysqli($db_config[$link_type][$server_id]['hostname'],$db_config[$link_type][$server_id]['username'],$db_config[$link_type][$server_id]['password'],$database);
        
        $char_set = $db_config[$link_type][$server_id]['char_set'];      
        if($this->conn != FALSE) {
            mysqli_query($this->conn,"set names $char_set");
        }
    }
    ...
    public function escape_row(&$row){
        if(is_object($row)){
            foreach($row as $k=>$v){
                $row->$k = mysqli_real_escape_string($v);
            }
        }else if(is_array($row)){
            foreach($row as $k=>$v){
                $row[$k] = mysqli_real_escape_string($v);

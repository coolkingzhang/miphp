...
class mysqlpdoAdapter
{
	...
	*/
	function connect($link_type = 'read')
	{
		...
		try
		{
			$this->_dsn 	= "mysql:host=".$db_config[$link_type][$server_id]['hostname'].";dbname=$database";
			$this->_name 	= $db_config[$link_type][$server_id]['username'];
			$this->_pass 	= $db_config[$link_type][$server_id]['password']; 
			$this->pdo 		= new PDO($this->_dsn,$this->_name,$this->_pass,$this->_condition);
			self::exec('SET NAMES '.$char_set);
		} 
		catch(Exception $e) 
		{
